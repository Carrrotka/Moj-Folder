...
# Load the data into a pandas DataFrame
    ...
    def split_data(stock, lookback):
        ...
        index_raw2=[]
        for t in range(1,len(data_raw)+201):
        ...
        x_test2 = adam2[train_set_size:,:-1]
            
        #print(x_test)
        y_test = data[train_set_size:,-1,:]
        
        return [x_train, y_train, x_test, y_test,x_test2,index_raw2]
    ...
    x_train, y_train, x_test, y_test, x_test2,index_raw2 = split_data(price_stock, lookback)
    print(len(x_test),len(x_test2))
...
# make predictions
    ...
    empty_rows = pd.DataFrame(np.nan, index=range(200), columns=price_stock.columns)
    print(len(price_stock))
    price_stock = price_stock._append(empty_rows)
    print(len(price_stock))
    ...
    #fig.show()
    return(float(result[1].iloc[-2])/float(result[2].iloc[-202]))
